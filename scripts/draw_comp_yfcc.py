...
root = '/home/eugenelee/Downloads/SAPT/code/yfcc_comp'
...
def draw_method(results, dist, draw_circle, s, method, index_list=None, return_text=False, blend_weight=1.0):
    ...
    for i in tqdm.tqdm(_list):
        ...
        ret.append(vis)
        # cv2.imwrite(f"{root}/{method}/{i:04d}.jpg", vis)
...
def draw_all_method():
    ...
    for method in ['superglue', 'loftr', 'ours', 'aspanformer']:
        ...
        results = list(np.load(f"{root}/yfcc_{method}.npy", allow_pickle=True))
        draw_method(results, dist, draw_circle, s, method)

def draw_comp():
    dist = 1
    # method='loftr'
    # index_list = [15,56,91,177,432,640,809]
    index_list = [10, 15, 64, 91, 177, 180, 185, 362, 469, 490, 554, 640, 719, 864, 915, 956, 1076, 1170, 1238, 1374, 1379, 1499, 1510, 1636, 1649, 1813, 1963, 2010, 2035, 2076, 2109, 2472, 2501, 2623, 2674, 2752, 2805, 2915, 3072, 3103, 3259, 3307, 3313, 3332, 3364, 3387, 3435, 3440, 3521, 3553, 3597, 3712, 3784, 3802]
    # index_list = [864, 3313]
    # index_list = [3420]
    comp_results = []
    text_list = []
    ...
    for j in range(len(comp_results[0])):
        ...
        # cv2.imshow('', vis)
        # cv2.waitKey(0)
        cv2.imwrite(f'{root}/final/{index_list[j]:04d}.png', vis)
