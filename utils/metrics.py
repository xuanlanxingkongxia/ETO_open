...
def compute_pose_error(kp1, kp2, intrinsic1, intrinsic2, extrinsic1,
    ...
    if kp1.shape[0] < 15:
        return np.inf, np.inf, np.array([np.inf]), np.array([np.inf])
    ...
    E, mask = cv2.findEssentialMat(
        kp1, kp2, np.eye(3), threshold=norm_thresh, prob=1-1e-5, method=cv2.RANSAC)
    result_mask = result[mask.astype(float).reshape(-1) > 0.5]
    ...
    return error_R, error_t, result, result_mask
